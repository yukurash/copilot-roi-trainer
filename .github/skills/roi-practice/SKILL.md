...
1. 親Agentがプロファイルを読む。
2. 受講者へ技術エリアと業界・業務領域を確認する。依頼に明示された項目は聞き直さない。
3. 確認結果を含むScenario RequestをScenario Designerへ渡す。
4. Executive Challengerを各ターン独立に呼び、親Agentが会話全文を再送する。
5. 本人の自己振り返りを採点前に取得する。
6. Evaluatorを過去情報なしで呼ぶ。
7. Reflection Coachへ自己評価、独立評価、前回目標を渡す。
8. `active-session.json`を作成し、`complete-session.ps1`で検証・保存・プロファイル更新を行う。
